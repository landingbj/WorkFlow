...
export class WorkflowRuntimeServerClient implements IRuntimeClient {
  ...
  public async [FlowGramAPIName.TaskRun](input: TaskRunInput): Promise<TaskRunOutput | undefined> {
    try {
      ...
      const response = await fetch('/workflow/taskRun', {
        method: 'POST',
        headers: {
  ...
  public async [FlowGramAPIName.TaskReport](
    ...
    try {
      const response = await fetch(`/workflow/taskReport?taskID=${input.taskID}`, {
        method: 'GET',
        redirect: 'follow',
      });
  ...
  public async [FlowGramAPIName.TaskResult](
    ...
    try {
      const response = await fetch(`/workflow/taskResult?taskID=${input.taskID}`, {
        method: 'GET',
        redirect: 'follow',
      });
